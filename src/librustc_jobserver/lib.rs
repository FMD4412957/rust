...
pub struct Helper {
    ...
    waiter: Arc<Condvar>,
    helper: Mutex<jobserver_crate::HelperThread>,
}

impl Helper {
    fn new() -> Self {
        let tokens = Arc::new(Mutex::new(TokenCounts { used: 1, pending: 0 }));
        let waiter = Arc::new(Condvar::new());
        let tokens_thread = tokens.clone();
        let waiter_thread = waiter.clone();
        let helper = GLOBAL_CLIENT
            ...
            .into_helper_thread(move |token| {
                ...
                let mut tokens = tokens_thread.lock().unwrap();
                tokens.pending += 1;
                waiter_thread.notify_one();
            })
            .expect("spawned helper");
        Helper { tokens, waiter, helper: Mutex::new(helper) }
    }

    fn acquire_token_from_request(&self) {
        // We essentially duplicate acquire_token below but never emit a request
        // for additional tokens.
        log::trace!("{:?} acquiring token from request", std::thread::current().id());
        let mut tokens = self.tokens.lock().unwrap();
        loop {
            if tokens.used == 0 {
                log::trace!("{:?} acquired implicit token", std::thread::current().id());
                tokens.used = 1;
                return;
            } else if tokens.pending > 0 {
                // forget the token so as to not drop
                tokens.pending -= 1;
                tokens.used += 1;
                log::trace!("{:?} acquired real token", std::thread::current().id());
                return;
            }
            tokens = self.waiter.wait(tokens).unwrap();
        }
    }

    fn acquire_token(&self) {
        log::trace!("{:?} acquiring token", std::thread::current().id());
        let mut tokens = self.tokens.lock().unwrap();
    ...
    fn release_token(&self) {
        log::trace!("{:?} releasing token", std::thread::current().id());
        let mut tokens = self.tokens.lock().unwrap();
        tokens.used -= 1;
        if tokens.used == 0 {
            log::trace!("{:?} released implicit token", std::thread::current().id());
            self.waiter.notify_one();
        } else {
            log::trace!("{:?} released real token", std::thread::current().id());
            if should_notify() {
                eprintln!(
                    "{}",
                    as_json(&JobserverNotification { jobserver_event: Event::Release })
                );
            } else {
    ...
    pub fn request_token(&self) {
        ...
        self.helper.lock().unwrap().request_token();
    }
}
...
pub fn helper_channel() -> &'static Helper {
    &*HELPER
}
...
pub fn acquire_from_request() -> Acquired {
    HELPER.acquire_token_from_request();
    Acquired(())
}

pub fn acquire_thread() {
    HELPER.acquire_token();
}
